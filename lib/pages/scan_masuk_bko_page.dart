...
import 'package:cobra_apps/pages/absen_masuk_bko_page.dart';
import 'dart:convert';
...
class ScanMasukBkoPage extends ConsumerStatefulWidget {
  final Map<String, dynamic>? data;
  const ScanMasukBkoPage({super.key, this.data});

  @override
  ConsumerState<ScanMasukBkoPage> createState() => _ScanMasukBkoPageState();
}

class _ScanMasukBkoPageState extends ConsumerState<ScanMasukBkoPage> {
  ...
  void reassemble() {
    super.reassemble();
    // nothing required for DetectorView
  }
  ...
  void dispose() {
    ...
    // No camera controller to dispose when using DetectorView
  ...
  // DetectorView supplies frames; we'll handle them in _processImage
        ...
        if (!ref.read(scanMasukProvider).navigated && data['success'] == true) {
          ...
          if (maxDistance != null &&
            ...
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (_) => AbsenMasukBkoPage(data: data)),
            );
          }
        }
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () {
                              try {
                                ...
                                log('User requested rescan (Scan Masuk BKO)');
                              } catch (e) {
